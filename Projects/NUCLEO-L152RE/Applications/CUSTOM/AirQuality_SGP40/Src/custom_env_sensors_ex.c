...
int32_t CUSTOM_ENV_SENSOR_Get_DRDY_Status(uint32_t Instance, uint32_t Function, uint8_t *Status)
{
  ...
  switch (Instance)
  {
    ...
    case CUSTOM_SGP40_0:
      if ((Function & ENV_GAS) == ENV_GAS)
      {
        if (SGP40_GAS_Get_DRDY_Status(EnvCompObj[Instance], Status) != BSP_ERROR_NONE)
        {
          ret = BSP_ERROR_COMPONENT_FAILURE;
        }
        else
...
#if (USE_CUSTOM_ENV_SENSOR_SGP40_0 == 1)
    case CUSTOM_SGP40_0:
      if (SGP40_GAS_Set_Humidity(EnvCompObj[Instance], Value) != BSP_ERROR_NONE)
      {
        ret = BSP_ERROR_COMPONENT_FAILURE;
      }
      else
...
#if (USE_CUSTOM_ENV_SENSOR_SGP40_0 == 1)
    case CUSTOM_SGP40_0:
      if (SGP40_GAS_Set_Temperature(EnvCompObj[Instance], Value) != BSP_ERROR_NONE)
      {
        ret = BSP_ERROR_COMPONENT_FAILURE;
      }
      else
