...
#define GAS_UNICLEO_ID UNKNOWN_UNICLEO_ID
#endif
 ...
 */
{
  ...
  switch (Msg->Data[2])   /* CMD */
  {
    ...
    case CMD_GAS_Init:
      ...
      BUILD_REPLY_HEADER(Msg);
      Serialize_s32(&Msg->Data[3], GAS_UNICLEO_ID, 4);
      Msg->Len = 3 + 4;
      UART_SendMsg(Msg);
      break;
    ...
    case CMD_Offline_Data:
      ...
      for (i = 0; i < msg_count; i++)
      {
        ...
        memcpy(&OfflineData[OfflineDataWriteIndex].gas, &Msg->Data[msg_offset + 16], 4);
    ...
    case CMD_Get_App_Info:
      ...
      Serialize_s32(&Msg->Data[3], AppFreq, 4);
...
{
  const uint8_t string_pointer_shift = strlen("ST EnvXX v"); /* Shift string pointer by this amount */
  char *lib_version_num;
  char lib_version_string[64];
  int lib_version_len = 0;
  const char ps[] = {"MEMS shield demo,30,"FW_VERSION",%s,"BOARD_NAME};

  EnvAQ_manager_get_version(lib_version_string, &lib_version_len);
