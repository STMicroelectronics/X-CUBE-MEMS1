...
static void MX_PedometerWrist_Init(void);
static void MX_PedometerWrist_Process(void);
...
static void MX_PedometerWrist_Init(void)
{
  ...
  /* PedometerWrist API initialization function */
  MotionPW_manager_init();

  /* Activity Recognition Wrist API initialization function */
  MotionAW_manager_init();

  /* OPTIONAL */
  /* Get library version */
  MotionPW_manager_get_version(LibVersion, &LibVersionLen);
...
/**
  * @brief  Pedometer Wrist data handler
  * @param  Msg the Pedometer Wrist data part of the stream
  * @retval None
  */
static void PW_Data_Handler(TMsg *Msg)
{
  ...
  {
    /* Fill input data */
    mpw_data_in.AccX = maw_data_in.AccX = (float)AccValue.x / 1000.0f;    /* Convert from [mg] to [g] */
    mpw_data_in.AccY = maw_data_in.AccY = (float)AccValue.y / 1000.0f;    /* Convert from [mg] to [g] */
    mpw_data_in.AccZ = maw_data_in.AccZ = (float)AccValue.z / 1000.0f;    /* Convert from [mg] to [g] */
