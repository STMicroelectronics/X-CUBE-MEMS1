...
#define PRESS_ODR  10.0f /* ODR > 2Hz */
...
static void MX_AirplaneDetection_Init(void);
static void MX_AirplaneDetection_Process(void);
static void AD_Data_Handler(TMsg *Msg);
...
static void MX_AirplaneDetection_Init(void)
{
  ...
  /* AirplaneDetection API initialization function */
  MotionAD_manager_init((int)ALGO_FREQ);

  /* OPTIONAL */
  /* Get library version */
  MotionAD_manager_get_version(LibVersion, &LibVersionLen);
...
static void MX_AirplaneDetection_Process(void)
{
  ...
  if (SensorReadRequest == 1U)
  {
    ...
    /* AirplaneDetection specific part */
    AD_Data_Handler(&msg_dat);
...
/**
 * @brief  Airplane Detection data handler
 * @param  Msg the Airplane Detection data part of the stream
 * @retval None
 */
static void AD_Data_Handler(TMsg *Msg)
{
  uint32_t elapsed_time_us = 0U;
  MAD_input_t data_in = {.Acc[0] = 0.0f, .Acc[1] = 0.0f, .Acc[2] = 0.0f, .Press = 0, .Temp = 0.0f};
  static MAD_output_t data_out;

  if ((SensorsEnabled & ACCELEROMETER_SENSOR) == ACCELEROMETER_SENSOR &&
      (SensorsEnabled & PRESSURE_SENSOR) == PRESSURE_SENSOR &&
      (SensorsEnabled & TEMPERATURE_SENSOR) == TEMPERATURE_SENSOR)
  {
    /* Fill input data */
    data_in.Acc[0] = (float)AccValue.x / 1000.0f;    /* Convert from [mg] to [g] */
    data_in.Acc[1] = (float)AccValue.y / 1000.0f;    /* Convert from [mg] to [g] */
    data_in.Acc[2] = (float)AccValue.z / 1000.0f;    /* Convert from [mg] to [g] */
    data_in.Press = (unsigned int)PressValue * 100;  /* Convert from [hPa, mBar] (e.g. 1000) to [Pa] (e.g. 100000) */
    data_in.Temp = TempValue;

    /* Run Airplane Detection algorithm */
    BSP_LED_On(LED2);
    DWT_Start();
    MotionAD_manager_run(&data_in, &data_out);
...
static void TIM_Config(uint32_t Freq)
{
  const uint32_t tim_counter_clock = 4000; /* TIM counter clock 4 kHz */
