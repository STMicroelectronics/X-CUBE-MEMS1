...
#define ACC_ODR  100.0f
#define PRESS_ODR  25.0f /* ODR >= 25Hz */
...
static void MX_FallDetection_Init(void);
static void MX_FallDetection_Process(void);
...
static void MX_FallDetection_Init(void)
{
  ...
  /* OPTIONAL */
  /* Get library version */
  MotionFD_manager_get_version(LibVersion, &LibVersionLen);
...
static void FD_Data_Handler(TMsg *Msg)
{
  ...
  MFD_input_t data_in = {.AccX = 0.0f, .AccY = 0.0f, .AccZ = 0.0f, .Press = 0.0f};
  ...
  if ((SensorsEnabled & ACCELEROMETER_SENSOR) == ACCELEROMETER_SENSOR)
  {
    ...
    {
      /* Fill input data */
      data_in.AccX = (float)AccValue.x;    /* [mg] */
      data_in.AccY = (float)AccValue.y;    /* [mg] */
      data_in.AccZ = (float)AccValue.z;    /* [mg] */
      data_in.Press = PressValue;          /* [hPa] */

      /* Run Fall Detection algorithm */
      BSP_LED_On(LED2);
      DWT_Start();
      MotionFD_manager_run(&data_in, &data_out);
