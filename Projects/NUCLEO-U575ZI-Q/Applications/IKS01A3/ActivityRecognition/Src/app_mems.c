...
static void MX_ActivityRecognition_Init(void);
static void MX_ActivityRecognition_Process(void);
...
void MX_MEMS_Process(void)
{
  ...
  MX_ActivityRecognition_Process();
...
static void MX_ActivityRecognition_Init(void)
{
  ...
  /* OPTIONAL */
  /* Get library version */
  MotionAR_manager_get_version(LibVersion, &LibVersionLen);
...
static void AR_Data_Handler(TMsg *Msg)
{
  ...
  static MAR_output_t data_out;

  if ((SensorsEnabled & ACCELEROMETER_SENSOR) == ACCELEROMETER_SENSOR)
  {
    /* Fill input data */
    data_in.acc_x = (float)AccValue.x / 1000.0f;    /* Convert from [mg] to [g] */
    data_in.acc_y = (float)AccValue.y / 1000.0f;    /* Convert from [mg] to [g] */
    data_in.acc_z = (float)AccValue.z / 1000.0f;    /* Convert from [mg] to [g] */

    /* Run Activity Recognition algorithm */
    BSP_LED_On(LED2);
    DWT_Start();
    MotionAR_manager_run(&data_in, &data_out, Timestamp);
