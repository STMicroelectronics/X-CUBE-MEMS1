...
static void MX_ApproachLeave_Init(void);
static void MX_ApproachLeave_Process(void);
static void AL_Data_Handler(Msg_t *Msg);
...
static void MX_ApproachLeave_Init(void)
{
  ...
  /* ApproachLeave API initialization function */
  InfraredAL_manager_init();

  /* OPTIONAL */
  /* Get library version */
  InfraredAL_manager_get_version(LibVersion, &LibVersionLen);
...
static void MX_ApproachLeave_Process(void)
{
  ...
  if (SensorReadRequest == 1U)
  {
    ...
    {
      /* ApproachLeave specific part */
      AL_Data_Handler(&msg_dat);
    }
...
/**
  * @brief  Approach Leave data handler
  * @param  Msg the Approach Leave data part of the stream
  * @retval None
  */
static void AL_Data_Handler(Msg_t *Msg)
{
  uint32_t elapsed_time_us = 0U;
  IAL_input_t data_in = {.t_obj = 0};
  static IAL_output_t data_out;
  ...
  if ((SensorsEnabled & TEMPERATURE_SENSOR) == TEMPERATURE_SENSOR)
  {
    ...
    BSP_LED_On(LED2);
    DWT_Start();
    InfraredAL_manager_run(&data_in, &data_out);
    elapsed_time_us = DWT_Stop();
    BSP_LED_Off(LED2);

    Msg->Data[11] = (uint8_t)data_out.pres_flag;
    Msg->Data[12] = (uint8_t)data_out.pres_conf;

    Serialize_s32(&Msg->Data[13], (int32_t)elapsed_time_us, 4);
  }
}
...
static void Infrared_Sensor_Handler(Msg_t *Msg)
{
  if ((SensorsEnabled & TEMPERATURE_SENSOR) == TEMPERATURE_SENSOR)
  {
    ...
    Serialize_s32(&Msg->Data[7], (int32_t)TObjRaw, 4);
  }
}
