 ...
 */
{
  ...
  float algo_freq = (float)AlgoFreq;
  ...
  switch (Msg->Data[2])   /* CMD */
  {
    ...
    case CMD_Calibration_Mode:
      ...
      switch ((MTL2_MODE_t)Msg->Data[3])
      {
        case MTL2_SINGLE_PLANE:
        case MTL2_DUAL_PLANE:
          MotionTL2_manager_init();
          MTL2_Knobs.mode = (MTL2_MODE_t)Msg->Data[3];
          MotionTL2_SetKnobs(&MTL2_Knobs);
          break;
...
{
  const uint8_t string_pointer_shift = strlen("ST MotionXXX v"); /* Shift string pointer by this amount */
  char *lib_version_num;
  char lib_version_string[64];
  int lib_version_len = 0;
  const char ps[] = {"MEMS shield demo,28,"FW_VERSION",%s,"BOARD_NAME};

  MotionTL2_manager_get_version(lib_version_string, &lib_version_len);
